...
2. `cols`: Usually specified with a vector of variable names.
  + All variable names in in the provided vector of names must be quoted, and no variable ranges are permitted. 
  + No variable ranges.
3. No character strings for values passed to `rows` and `cols`. Instead enter the conditions for both rows and columns, which can make the entire expression quite large.
...
Here with the traditional use of `Extract`, specify rows of data only when the value of _Dept_ is _ADMIN_. 

```{r}
d[d$Dept=="ADMN", c('Gender', 'Dept', 'Salary')]
```

The result provides what was asked for, and also when _Dept_ is `<NA>`, which was not asked for. The requested value of _ADMIN_ is not `<NA>`.

The `.()` function provides what was requested.

```{r}
d[.(Dept=="ADMN"), .(Gender:Salary)]
```

If rows with the value of the variable missing are also requested, then `.()` provides that information only when requested, such as from the base R function `is.na()`.
```{r}
