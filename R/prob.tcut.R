...
function(df, alpha=0.05, dig.dec=3, y.axis=FALSE,
  ...
  y <- dt(x,df=df)
  polygon(c(x1min,x,x1max), c(0,y,0), col=color.tail, border=nrm.color)
  ...
  y <- dt(x,df=df)
  polygon(c(x2min,x,x2max), c(0,y,0), col=color.tail, border=nrm.color)
  ...
  x <- seq(xmin,xmax,length=200)
  y <- dnorm(x)
  lines(x,y, type="l", lwd=2, col=nrm.color, las=1, main="", xlab="z", ylab="")
  abline(h=0)

  # fill
  txtht <- 0.175
  rlbl <- paste(toString(round(100*(1-alpha),0)), "%", sep="")
  text(0, txtht, rlbl, cex=1.4, col=nrm.color)

  # Left Tail
  lines(c(x1max,x1max), c(0,.25), col=nrm.color)
  text(x1max, .26, col=nrm.color, cex=1.1, .fmt(x1max, dig.dec, w=5))

  # Right Tail
  lines(c(x2min,x2min), c(0,.25), col=nrm.color)
  text(x2min, .26, col=nrm.color, cex=1.1, .fmt(x2min, dig.dec, w=5))
