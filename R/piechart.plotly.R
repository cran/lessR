piechart.plotly <- function(
  ...
  if (is.null(dim(x)) || length(dim(x)) == 0L || length(dim(x)) == 1L) {
    ...
    if (!is.finite(alpha_fill)) alpha_fill <- 1
    ...
    built <- tryCatch(plotly::plotly_build(plt), error = function(e) {
      warning("piechart.plotly: plotly_build() failed; slice borders may not render. ",
  ...
  build_pie_grid <- function() {
    ...
    plt <- plotly::layout(plt, uniformtext = list(minsize = 10, mode = "show"))
    plt
  } # end build_pie_grid
