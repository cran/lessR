...
function(lm.out, dname="mydata", digits.d=NULL, show.R=FALSE) {
  ...
  se <- sm$sigma
  tx[length(tx)+1] <- paste("Standard deviation of residuals: ", .fmt(se,digits.d-1),
  ...
  tx[length(tx)+1] <- paste("  value is 2*t-cutoff*", .fmt(se,digits.d-1), 
    ", with a 95% interval t-cutoff of ", .fmt(tcut,3), sep="")
  tx[length(tx)+1] <- paste("95% range of variation: ", .fmt(2*tcut*se,digits.d-1), sep="")
  ...
  return(list(tx=tx, se=sm$sigma, Rsq=sm$r.squared, Rsqadj=sm$adj.r.squared))
