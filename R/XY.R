XY <-
function(x, y=NULL, data=d, filter=NULL,  # keep y=NULL for if x is a ts
  ...
  if (!is.null(ts_unit)  &&  !is.null(substitute(facet))) {
    cat("\n"); stop(call.=FALSE, "\n------\n",
  ...
  if (!is.null(fit_new))
    fit_new <- base::sort(fit_new)  # ensure ascending order of fit_new
...
# old stuff ---------------------------------------------------------------
  ...
  if (!is.null(dots)) if (length(dots) > 0) {
    for (i in seq_along(dots)) {
      ...
      if (names(dots)[i] == "run") {
        cat("\n"); stop(call.=FALSE, "\n------\n",
          "Parameter dropped: run. Specify the 1st variable, the x-variable,\n",
          "  as .Index to automatically create the index variable.\n",
 ...
 if (is.null(fill))
  ...
  if (missing(vbs_ratio)) if (!violin)  # wider box if no violin
    vbs_ratio <- ifelse (y.miss || facet.miss, vbs_ratio*3.75, vbs_ratio*5)
...
# --- scatterplot matrix detection -----------------------------------

    same_xy_expr <- FALSE
    if (!y.miss) {  # compare *expressions* exactly as typed by the user
      same_xy_expr <- identical(substitute(x), substitute(y))
    }

    spmat <- FALSE
    if (!y.miss && same_xy_expr && all(is.nmb))  # do a scatterplot matrix
      spmat <- TRUE
    else {
      if (y.miss && all(is.nmb)) {
        cat("\n"); stop(call.=FALSE, "\n------\n",
          "Scatterplot matrices in lessR have a new interface.\n",
          "  Old shortcut:  XY(Salary, Age, Years)\n",
          "  New syntax:    XY(x = c(Salary, Age, Years),\n",
          "                    y = c(Salary, Age, Years))\n\n",
          "The y-missing shortcut is deprecated. Specify the\n",
          "same vector of numeric variables for both x= and y=.\n")
      }
    }
  ...
  if (!facet.miss && cat.x && cat.y) {
    cat("\n"); stop(call.=FALSE, "\n------\n",
      "Parameter  facet  does not work with bubble plots,\n",
...
# --- resolve facet --------------------------------------------------
  ...
  # ---------------------------------
  #  manage regular-R or PDF graphics
  if (!Trellis) {
    if (!is.null(pdf_file)) {
...
# set object and values where needed --------------------------------------
  ...
  # y.call to 0   >> Move all no y to X()
  ...
  pt.trans <- trans
  if (object == "bubble"  &&  trans.miss) pt.trans <- 0.4

  if (!stat_x.miss) n.yvar <- 1

  if (y.miss  &&  !date.var  &&  object != "bubble" && !stat_x.miss) {

    if (!cat.x) {  # freq polygon

      ssstuff <- .ss.numeric(x.call[,1], digits_d=digits_d, brief=TRUE)

      # get x.call binned  values and corresponding y.call count values
      # do the actual freq poly in .plt.main
      hist.cumul <- ifelse(cumulate, "on", "off")
      reg <- "snow2"  # applies to cumulative histogram
      digits_d <- .max.dd(x.call[,1])
      h <- .hst.main(x.call[,1], by=NULL, by.name=NULL, n.by=1,
         pt.fill, pt.color, pt.trans, reg,
         rotate_x, rotate_y, offset,
         breaks, bin_start, bin_width, bin_end,
         proportion, counts=NULL, hist.cumul,
         xlab, ylab, main, sub,
         bm.adj=0, lm.adj=0, tm.adj=0, rm.adj=0,
         axis_fmt="K", axis_x_pre="", axis_y_pre="",
         use_plotly=FALSE, digits_d=NULL,
  ...
  if (size.miss)  # pt.size not set yet
    pt.size <- ifelse (.Platform$OS.type == "windows", 1.00, 1.00)
  else {  # size had been set
    if (length(size) == 1)
      scale.pt <- ifelse (.Platform$OS.type == "windows", 1.00, 1.00)
    else {  # size var
  ...
  if (Trellis && do_plot) {
    if (T.type %in% c("cont", "cont_cont", "cont_cat")) {
      ...
      if (y.miss  &&  !run  &&  T.type != "cont_cont") {
        ...
        message("Warning: In the future, run VBS plots from X()\n")
...
# pivot tables ------------------------------------------------------------
      ...
      if (!date.var && !run) {  # no time vars, VBS only

        x_vec <- x.call[[ x.name ]]   # e.g., data[["Salary"]]
      ...
      if (.allow.interactive()) {
        .viewer_notice_once(plot_name = type, window_target = "Plots")
      }
    }  # end T.type != "dot
