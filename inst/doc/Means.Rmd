...
output: 
  rmarkdown::html_vignette:
    toc: true
vignette: >
  %\VignetteIndexEntry{3. Models: Mean Inference}
...
The aggregated data is stored in the data frame named _dm_, so explicitly identify with the `data` parameter. The computed mean of _breaks_ variable in the _dm_ data frame from the previous call to `pivot()` is named *breaks_mn* by default.
...
```
In this example use the __lessR__ `to` function to create that vector without needed to individual list each variable.
