...
Plot(Weight, MPGcity, by1=Cylinders, by2=Transmission, by=Source)
```
...
To avoid over-plotting, the plott of two categorical variables results in a bubble plot of their joint frequencies.
...
An interactive visualization lets the user in real time change parameter values to change characteristics of the visualization. To create an interactive two-variable scatterplot of continuous variables that displays the corresponding parameters, run the function `interact()` with `"ScatterPlot"` specified. 

```
interact("ScatterPlot")
```
To create an interactive Trellis plot as a combined violion, box, and scatter plot with the five values of Dept that displays the corresponding parameters, run the function `interact()` with `"Trellis"` specified.
